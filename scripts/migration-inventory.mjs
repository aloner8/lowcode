...
export const INVENTORY_SOURCES = [
  { key: 'accounts', table: 'platform_users', fingerprint: "jsonb_build_array(id, username, email, global_role, is_active, updated_at)", mapping: "SELECT COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM public.customer_memberships m WHERE m.user_id=s.id))::text AS mapped_count FROM public.platform_users s" },
  { key: 'platforms', table: 'platforms', fingerprint: "jsonb_build_array(id, platform_slug, is_published, updated_at)", mapping: "SELECT COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM public.templates t WHERE t.legacy_platform_id=s.id))::text AS mapped_count FROM public.platforms s" },
...
export const REFERENCE_CHECKS = [
  { key: 'apps_without_source_or_template', sql: "SELECT COUNT(*)::text AS violation_count FROM public.apps WHERE platform_id IS NULL AND template_id IS NULL" },
  { key: 'page_component_missing_page', sql: "SELECT COUNT(*)::text AS violation_count FROM public.platform_pages_components i LEFT JOIN public.platform_pages p ON p.id=i.platform_page_id WHERE p.id IS NULL" },
  { key: 'page_component_missing_component', sql: "SELECT COUNT(*)::text AS violation_count FROM public.platform_pages_components i LEFT JOIN public.platform_components c ON c.id=i.platform_component_id WHERE c.id IS NULL" },
  { key: 'domain_missing_app', sql: "SELECT COUNT(*)::text AS violation_count FROM public.app_domains d LEFT JOIN public.apps a ON a.id=d.app_id WHERE a.id IS NULL" },
  { key: 'binding_missing_scope', sql: "SELECT COUNT(*)::text AS violation_count FROM public.app_service_bindings b LEFT JOIN public.platforms p ON p.id=b.platform_id LEFT JOIN public.apps a ON a.id=b.app_id WHERE p.id IS NULL OR (b.app_id IS NOT NULL AND a.id IS NULL)" },
];

const readSource = async (client, source) => {
  const exists = await client.query('SELECT to_regclass($1) IS NOT NULL AS present', [`public.${source.table}`]);
  if (!exists.rows[0]?.present) return { key: source.key, table: source.table, status: 'MISSING', count: null, mapped: null, unresolved: null, checksum: null };
...
export async function createMigrationInventory(client, now = new Date()) {
  ...
  try {
    ...
    const sources = [];
    for (const source of INVENTORY_SOURCES) sources.push(await readSource(client, source));
    const references = [];
    for (const check of REFERENCE_CHECKS) {
      const result = await client.query(check.sql);
      references.push({ key: check.key, violations: Number(result.rows[0].violation_count) });
    }
    const tenants = await client.query(`SELECT id AS app_id, app_slug, tenant_db_name
      FROM public.apps ORDER BY app_slug`);
    await client.query('COMMIT');
    const unresolved = sources.reduce((total, source) => total + (source.unresolved ?? 0), 0);
    const violations = references.reduce((total, reference) => total + reference.violations, 0);
    return {
