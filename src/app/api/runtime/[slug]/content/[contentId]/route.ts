...
export async function GET(
  _request: Request,
  context: { params: Promise<{ slug: string; contentId: string }> },
) {
  const { slug, contentId } = await context.params;
  const result = await getCoreDb().query<ContentPlatformRow>(
    `SELECT COALESCE((SELECT jsonb_agg(pp.page_config || jsonb_build_object(
     ...
     studio_forms, studio_collections, content_updated_at FROM public.platforms WHERE platform_slug=$1`,
    [slug],
  );
  if (!result.rowCount)
    return NextResponse.json(
      { error: "Runtime application not found" },
      { status: 404 },
    );
  ...
  const pages = Array.isArray(platform.studio_pages)
    ...
    : [];
  const forms = Array.isArray(platform.studio_forms)
  ...
  if (page) {
    ...
    kind = "app-component";
    label = collectionComponent.label || collectionComponent.id;
    componentTree = collectionComponent.componentTree;
  } else if (collection) {
    kind = "collection";
    label = collection.name || collection.id;
    componentTree = collection.components?.find(
      (item) => item.componentTree?.length,
    )?.componentTree;
  } else {
    const allTrees = [
      ...pages.map((item) => item.componentTree || []),
      ...forms.map((item) => item.componentTree || []),
    ...
    const node = allTrees
      .map((tree) => findNode(tree, contentId))
      .find(Boolean);
  ...
  const dynamicHtml =
    ...
    componentTree[0].type === "DynamicHtmlComponent"
      ? componentTree[0]
      : undefined;
  const version = createHash("sha1")
    .update(JSON.stringify(componentTree))
    .digest("hex")
    .slice(0, 12);
  return NextResponse.json(
    {
      ...
      label,
      version,
      updatedAt: platform.content_updated_at.toISOString(),
      componentTree,
      styleSheet: page?.styleSheet,
      layoutRegions: page?.layoutRegions,
      html: dynamicHtml?.props?.content,
      data: dynamicHtml?.props?.data || {},
    },
