...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!result.rowCount) return NextResponse.json({ error: 'ไม่พบ Page ที่เลือกใน platform_pages' }, { status: 404 });
  return NextResponse.json({
    page: result.rows[0].page,
    platform: { id, platformName: result.rows[0].platform_name, platformSlug: result.rows[0].platform_slug, masterThemeConfig: result.rows[0].master_theme_config },
  });
}
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  if (!body.page || !Array.isArray(body.page.componentTree)) return NextResponse.json({ error: 'ข้อมูล Page ไม่ถูกต้อง' }, { status: 400 });

  const slug = decodeURIComponent(pageId);
  const title = typeof body.page.title === 'string' && body.page.title.trim() ? body.page.title.trim() : slug;
  ...
  const result = await getCoreDb().query(
    `UPDATE public.platform_pages
     SET title=$3, component_tree=$4::jsonb, page_config=$5::jsonb,
         seo=COALESCE($6::jsonb, seo), updated_at=NOW()
     WHERE platform_id=$1 AND page_slug=$2 RETURNING page_slug`,
    [id, slug, title, JSON.stringify(body.page.componentTree), JSON.stringify(pageConfig), body.page.seo === undefined ? null : JSON.stringify(body.page.seo)],
  );
  if (!result.rowCount) return NextResponse.json({ error: 'ไม่พบ Page ที่เลือกใน platform_pages' }, { status: 404 });
