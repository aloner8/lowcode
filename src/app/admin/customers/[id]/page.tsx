    ...
    <div className="d-flex flex-column gap-3">
      ...
      <section className="adm-card">
        ...
        <div className="table-responsive"><table className="adm-table">
          ...
          <tbody>{customer.apps.length ? customer.apps.map((app) => <tr key={app.id}><td><Link className="adm-cell-strong adm-link" href={`/admin/apps/${app.id}`}>{app.name}</Link><span className="adm-cell-sub d-block">{app.slug}</span>{app.isSuspended && <span className="adm-chip is-danger mt-1">App ถูกระงับ</span>}</td><td>{app.templateName}</td><td>{app.packageName}</td><td className="text-end"><AppRuntimeControl appId={app.id} initial={{ desiredState: app.desiredState, observedState: app.observedState, error: app.runtimeError, healthCheckedAt: app.healthCheckedAt, metrics: app.runtimeMetrics, metricsAt: app.runtimeMetricsAt }} /></td></tr>) : <tr><td colSpan={4} className="text-center adm-cell-sub">ยังไม่มี App</td></tr>}</tbody>
        </table></div>
      </section>
    </div>
  );
