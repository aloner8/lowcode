...
export interface JwtServiceConfig {
  algorithm: 'HS256'; issuer: string; audience: string;
  accessTokenTtlSeconds: number; secretEnvKey: string;
}

const encode = (value: object) => Buffer.from(JSON.stringify(value)).toString('base64url');
const signature = (value: string, secret: string) => createHmac('sha256', secret).update(value).digest('base64url');

export function issueJwt(claims: JwtClaims, config: JwtServiceConfig, secret: string, nowSeconds = Math.floor(Date.now() / 1000)) {
...
export function verifyJwt(token: string, config: JwtServiceConfig, secret: string, nowSeconds = Math.floor(Date.now() / 1000)): JwtPayload {
