...
const resolveCanonicalMenuItem = (item: SlideMenuItem): SlideMenuItem => {
  const canonical = canonicalMenuItems.find(
    (candidate) =>
...
export function SiteRuntimeView({
  ...
  >(undefined);
  const [selectedLayoutRegions, setSelectedLayoutRegions] = useState<
    Record<string, boolean> | undefined
  >(undefined);
  const [selectedMenuLabel, setSelectedMenuLabel] = useState<string>("");
  const [contentOutletMode, setContentOutletMode] = useState<
    "content" | "page"
  >("page");
  ...
  useEffect(() => {
    ...
    const requestedPath = pathname.startsWith(prefix)
      ? pathname.slice(prefix.length) || "/"
      : "/";
    const route = runtimeData.routes?.find(
      (item) =>
        item.path === requestedPath ||
        item.legacyPaths?.includes(requestedPath),
    );
    if (!route) return;
    if (route.targetType === "page" && route.targetId) {
      ...
      const authService = runtimeData.services?.find(
        (item) => item.id === "service.auth.jwt",
      );
      if (
        page &&
        route.targetId === authService?.bundle?.adminPageId &&
        !window.sessionStorage.getItem(`auth:${appSlug}:accessToken`)
      ) {
        const loginPage = runtimeData.pages?.find(
          (item) => item.id === authService.bundle?.loginPageId,
        );
        if (loginPage?.componentTree?.length) {
          setSelectedContent(loginPage.componentTree);
          setSelectedStyleSheet(loginPage.styleSheet);
          setSelectedLayoutRegions(loginPage.layoutRegions);
          setSelectedMenuLabel(loginPage.title || "Login");
          setContentOutletMode("page");
          window.history.replaceState({}, "", `/app/${appSlug}/login`);
        }
        return;
      }
      if (page?.componentTree?.length) {
        setSelectedContent(page.componentTree);
        setSelectedStyleSheet(page.styleSheet);
        setSelectedLayoutRegions(page.layoutRegions);
        setSelectedMenuLabel(route.label);
        setContentOutletMode("page");
      }
    } else if (route.targetType === "form" && route.targetId) {
      const form = runtimeData.forms?.find(
        (item) => item.id === route.targetId,
      );
      if (form?.componentTree?.length) {
        setSelectedContent(form.componentTree);
        setSelectedMenuLabel(route.label);
        setContentOutletMode("content");
      }
    } else if (route.targetType === "collection" && route.targetId) {
      const collection = runtimeData.collections?.find(
        (item) => item.id === route.targetId,
      );
      const view =
        collection?.components.find(
          (item) => item.type === "DataTableComponent",
        ) || collection?.components[0];
      if (view?.componentTree?.length) {
        setSelectedContent(view.componentTree);
        setSelectedMenuLabel(route.label);
        setContentOutletMode("content");
      }
    } else if (route.targetType === "external" && route.externalUrl)
  ...
  const handleActionTrigger = async (
    ...
    if (actionId === "auth.login.submit") {
      try {
        ...
        const response = await fetch(
          `/api/runtime/${encodeURIComponent(appSlug)}/auth/login`,
          {
            method: "POST",
        ...
        window.sessionStorage.setItem(
          `auth:${appSlug}:accessToken`,
          result.token,
        );
        const page = runtimeData.pages?.find(
          (item) => item.id === result.successPageId,
        );
        if (!page?.componentTree?.length)
          throw new Error(`Success Page '${result.successPageId}' not found`);
        setSelectedContent(page.componentTree);
        setSelectedStyleSheet(page.styleSheet);
        setSelectedLayoutRegions(page.layoutRegions);
        setSelectedMenuLabel(page.title || "Admin");
        setContentOutletMode("page");
        setLastAction(null);
        const route = runtimeData.routes?.find(
          (item) => item.targetType === "page" && item.targetId === page.id,
        );
        window.history.pushState(
          {},
          "",
          `/app/${appSlug}${route?.path || "/admin"}`,
        );
      } catch (error) {
        setLastAction(error instanceof Error ? error.message : "Login failed");
      }
      return;
    }
    if (actionId === "logout") {
      ...
      if (page?.componentTree?.length) {
        ...
        setSelectedStyleSheet(page.styleSheet);
        setSelectedLayoutRegions(page.layoutRegions);
        setSelectedMenuLabel(page.title || "Login");
        setContentOutletMode("page");
        setLastAction(null);
    ...
    if (actionId === "menu.select") {
      ...
      if (action?.type === "switchContent" && action.contentId) {
        try {
          ...
          const response = await fetch(
            `/api/runtime/${encodeURIComponent(appSlug)}/content/${encodeURIComponent(action.contentId)}`,
            { cache: "no-store" },
          );
          const result = (await response.json()) as {
          ...
          if (!response.ok || !result.componentTree?.length)
            throw new Error(result.error || "Content not found");
          const params = action.params || {};
          setSelectedContent(
            result.componentTree.map((node) => ({
              ...node,
              props: { ...node.props, ...params, appSlug, routeParams: params },
            })),
          );
          setSelectedStyleSheet(result.styleSheet);
          setSelectedLayoutRegions(result.layoutRegions);
          setSelectedMenuLabel(selectedMenu.label || action.contentId);
          setContentOutletMode(result.kind === "page" ? "page" : "content");
          setLastAction(null);
        } catch (error) {
          setLastAction(
            error instanceof Error ? error.message : "Unable to switch content",
          );
        }
        return;
      }
      if (action?.type === "navigatePage" && action.pageId) {
        ...
        if (page?.componentTree?.length) {
          ...
          setSelectedMenuLabel(
            selectedMenu.label || page.title || action.pageId,
          );
          setContentOutletMode("page");
          setLastAction(null);
      ...
      if (action?.type === "callApi" && action.url) {
        try {
          ...
          setLastAction(
            `API '${action.apiId || action.url}' completed: ${JSON.stringify(result).slice(0, 160)}`,
          );
          if (action.resultContentId) {
            const contentResponse = await fetch(
              `/api/runtime/${encodeURIComponent(appSlug)}/content/${encodeURIComponent(action.resultContentId)}`,
              { cache: "no-store" },
            );
            const content = (await contentResponse.json()) as {
              componentTree?: ComponentNode[];
            };
            if (content.componentTree)
              setSelectedContent(
                content.componentTree.map((node) => ({
                  ...node,
                  props: { ...node.props, data: result, appSlug },
                })),
              );
          }
        } catch (error) {
      ...
      if (action?.type === "triggerAction") {
        await handleActionTrigger(action.actionId, action.payload);
        return;
      }
      if (action?.type === "none") return;
      const routeId =
        action?.type === "openRoute" || action?.type === "navigateRoute"
          ? action.routeId
          : selectedMenu.routeId;
      const route = routeId
        ? runtimeData.routes?.find((item) => item.id === routeId)
        : undefined;
      if (route) {
        if (route.targetType === "external" && route.externalUrl) {
        ...
        if (route.targetType === "page" && route.targetId) {
          ...
          if (page?.componentTree?.length) {
            ...
            window.history.pushState(
              { routeId: route.id },
              "",
              `/app/${appSlug}${route.path === "/" ? "" : route.path}`,
            );
            return;
          }
        }
        ...
        if (route.targetType === "legacy" && route.legacyPaths?.[0]) {
          window.location.assign(route.legacyPaths[0]);
          return;
        }
      }
      ...
      if (resource?.componentType === "FormComponent" && resource.formId) {
        nodes = runtimeData.forms?.find(
          (form) => form.id === resource.formId,
        )?.componentTree;
      } else if (resource?.collectionId) {
        const collection = runtimeData.collections?.find(
          (item) => item.id === resource.collectionId,
        );
        const view = collection?.components.find(
          (item) =>
            item.id === resource.componentId ||
            item.type === resource.componentType,
        );
      ...
      if (nodes?.length) {
        ...
        setSelectedContent(
          nodes.map((node) => ({
            ...node,
            props: { ...node.props, ...params, routeParams: params },
          })),
        );
        ...
        setLastAction(
          `ยังไม่พบ Component binding สำหรับเมนู '${selectedMenu.label || selectedMenu.id}'`,
        );
      }
      return;
    }
        ...
        setSelectedContent(
          view.componentTree.map((node) => ({
            ...
            props: {
              ...
              recordId,
              data: row,
              routeParams: { mode: "preview", id: recordId },
            },
          })),
        );
        setSelectedMenuLabel(
          `Preview ${collection.name || collection.id}${recordId !== undefined ? ` #${recordId}` : ""}`,
        );
        setLastAction(null);
      } else
        setLastAction(
          `ไม่พบ ViewDetail สำหรับ Collection '${payload?.collectionId || "-"}'`,
        );
      return;
    }
    if (actionId === "edit" || actionId === "create") {
      ...
      const formId =
        ...
        (payload?.collectionId
          ? String(payload.collectionId).replace(/\.collection$/, ".form")
          : undefined);
      ...
      if (collection && form?.componentTree?.length) {
        ...
        setSelectedMenuLabel(
          `${actionId === "edit" ? "แก้ไข" : "เพิ่ม"} ${collection.name || collection.id}${recordId !== undefined ? ` #${recordId}` : ""}`,
        );
        setLastAction(null);
      } else {
        setLastAction(
          `ไม่พบ Standard Form Flow สำหรับ Collection '${payload?.collectionId || "-"}'`,
        );
      }
      return;
    }
    ...
    setLastAction(
      `Event '${actionId}' triggered with data: ${JSON.stringify(payload)}`,
    );
      ...
      >
        ...
        <div className="flex-grow-1">
          <RuntimeContentOutlet
            ...
            shellNodes={initialContent.filter(
              (node) => node.type === "SlideMenuComponent",
            )}
            ...
            styleSheet={
              selectedContent
                ? selectedStyleSheet
                : runtimeData.pageLayout.styleSheet
            }
            layoutRegions={
              selectedContent
                ? selectedLayoutRegions
                : runtimeData.pageLayout.layoutRegions
            }
