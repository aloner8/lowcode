...
const VisualNode: React.FC<{ node: StudioNode; selectedId: string | null; onSelect: (id: string) => void }> = ({ node, selectedId, onSelect }) => {
  if (node.kind === 'text') return <span onClick={(event) => { event.stopPropagation(); onSelect(node.id); }} style={selectedId === node.id ? { outline: '2px solid #4f46e5' } : undefined}>{node.text}</span>;
  if (node.kind === 'component') {
    const Target = COMPONENT_REGISTRY[node.componentRef?.id as keyof typeof COMPONENT_REGISTRY];
    return <div onClick={(event) => { event.stopPropagation(); onSelect(node.id); }} style={selectedId === node.id ? { outline: '2px solid #4f46e5', outlineOffset: '2px' } : undefined}>{Target ? <Target {...(node.attributes || {})}>{node.children?.map((child) => <VisualNode key={child.id} node={child} selectedId={selectedId} onSelect={onSelect}/>)}</Target> : <div className="alert alert-warning">Shared: {node.componentRef?.displayName || node.componentRef?.id}</div>}</div>;
  }
  ...
  return <Tag {...attributes} className={node.classList?.join(' ')} onClick={(event: React.MouseEvent) => { event.stopPropagation(); onSelect(node.id); }} style={selectedId === node.id ? { outline: '2px solid #4f46e5', outlineOffset: '2px' } : undefined}>{node.children?.map((child) => <VisualNode key={child.id} node={child} selectedId={selectedId} onSelect={onSelect}/>)}</Tag>;
};
...
export const HtmlStudioShell: React.FC<HtmlStudioShellProps> = ({ document: initialDocument, onSave, onClose, embedded = false, onDirtyChange }) => {
  ...
  return <div className={`${embedded ? 'w-100 h-100' : 'position-fixed top-0 start-0 w-100 h-100'} bg-dark d-flex flex-column`} style={embedded ? { minHeight: 'calc(100vh - 340px)' } : { zIndex: 3000 }}>
    <div className="d-flex align-items-center justify-content-between px-3 py-2 text-white border-bottom border-secondary"><div><strong>HTML Studio IDE</strong><span className="text-white-50 ms-2 small">{document.name}</span></div><div className="d-flex gap-2"><div className="btn-group btn-group-sm"><button className={`btn ${viewMode === 'visual' ? 'btn-primary' : 'btn-outline-light'}`} onClick={() => setViewMode('visual')}><Eye size={13}/> Visual</button><button className={`btn ${viewMode === 'split' ? 'btn-primary' : 'btn-outline-light'}`} onClick={() => setViewMode('split')}><LayoutPanelLeft size={13}/> Split</button><button className={`btn ${viewMode === 'source' ? 'btn-primary' : 'btn-outline-light'}`} onClick={() => setViewMode('source')}><Code2 size={13}/> Source</button></div><div className="btn-group btn-group-sm"><button className={`btn ${device === 'desktop' ? 'btn-light' : 'btn-outline-light'}`} onClick={() => setDevice('desktop')}><Monitor size={13}/></button><button className={`btn ${device === 'tablet' ? 'btn-light' : 'btn-outline-light'}`} onClick={() => setDevice('tablet')}><Tablet size={13}/></button><button className={`btn ${device === 'mobile' ? 'btn-light' : 'btn-outline-light'}`} onClick={() => setDevice('mobile')}><Smartphone size={13}/></button></div><button className="btn btn-success btn-sm" onClick={saveDocument}><Save size={13}/> Save Page</button>{!embedded && onClose && <button className="btn btn-outline-light btn-sm" onClick={onClose}><X size={15}/></button>}</div></div>
    <div className="d-flex flex-grow-1 overflow-hidden">
      <main className="flex-grow-1 bg-secondary bg-opacity-25 overflow-auto d-flex gap-2 p-2" onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; }} onDrop={(event) => { event.preventDefault(); if (!addExternalDrop(event) && draggedLibraryLabel) addLibraryItem(draggedLibraryLabel); setDraggedLibraryLabel(null); }}>{viewMode !== 'source' && <div className="bg-white shadow-sm mx-auto overflow-auto position-relative" style={{ width, maxWidth: viewMode === 'split' ? '50%' : '100%', transition: 'width .2s', minHeight: '100%' }}><div className="position-absolute top-0 start-50 translate-middle-x badge bg-dark bg-opacity-50 mt-2">Drop Component or Asset here</div><div className="p-3 pt-5" data-hs-scope={document.styleSheet.scopeId}>{artifact.cssText && <style>{artifact.cssText}</style>}{document.root.map((node) => <VisualNode key={node.id} node={node} selectedId={selectedId} onSelect={setSelectedId}/>)}</div></div>}{viewMode !== 'visual' && <div className="d-flex flex-column bg-white" style={{ width: viewMode === 'split' ? '50%' : '100%' }}><div className="d-flex justify-content-between align-items-center p-2 border-bottom"><span className="small fw-bold"><Braces size={13}/> HTML Source</span><button className="btn btn-primary btn-sm" onClick={applySource}>Apply Source</button></div><textarea className="form-control border-0 rounded-0 font-monospace flex-grow-1" value={source} onChange={(event) => setSource(event.target.value)} spellCheck={false}/>{sourceErrors.length ? <div className="alert alert-danger rounded-0 mb-0 py-2 small">{sourceErrors.join(' ')}</div> : null}</div>}</main>
      <aside className="bg-white border-start p-3 overflow-auto" style={{ width: 280 }}><div className="d-flex justify-content-between align-items-center mb-3"><strong className="small">Inspector</strong>{selected && <button className="btn btn-sm btn-outline-danger" onClick={() => { setDocument((current) => ({ ...current, root: removeNode(current.root, selected.id) })); setSelectedId(null); }}><Trash2 size={12}/></button>}</div>{selected ? <div className="d-flex flex-column gap-3"><div><label className="form-label small">Node ID</label><input className="form-control form-control-sm" value={selected.id} disabled/></div>{selected.kind === 'text' ? <div><label className="form-label small">Text</label><textarea className="form-control form-control-sm" value={selected.text || ''} onChange={(event) => updateSelected((node) => ({ ...node, text: event.target.value }))}/></div> : <><div><label className="form-label small">Tag</label><input className="form-control form-control-sm" value={selected.tag || ''} disabled/></div><div><label className="form-label small">Classes</label><input className="form-control form-control-sm" value={selected.classList?.join(' ') || ''} onChange={(event) => updateSelected((node) => ({ ...node, classList: event.target.value.split(/\s+/).filter(Boolean) }))}/></div>{selected.tag === 'img' && <><div><label className="form-label small">Image URL</label><input className="form-control form-control-sm" value={String(selected.attributes?.src || '')} onChange={(event) => updateSelected((node) => ({ ...node, attributes: { ...node.attributes, src: event.target.value } }))}/></div><div><label className="form-label small">Alt text</label><input className="form-control form-control-sm" value={String(selected.attributes?.alt || '')} onChange={(event) => updateSelected((node) => ({ ...node, attributes: { ...node.attributes, alt: event.target.value } }))}/></div></>}</>}</div> : <div className="text-muted small">Select an element on the canvas.</div>}<hr/><div className="small fw-bold mb-2">Diagnostics</div>{artifact.diagnostics.length ? artifact.diagnostics.map((item, index) => <div key={index} className={`alert py-1 px-2 small ${item.severity === 'error' ? 'alert-danger' : 'alert-warning'}`}>{item.message}</div>) : <div className="text-success small">No errors</div>}</aside>
    </div>
  </div>;
};
