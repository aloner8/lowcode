...
export const ComponentWorkshop: React.FC<ComponentWorkshopProps> = ({ pageName, component, collections, onBack, onSave }) => {
  ...
  return <div className="card border-0 shadow-sm h-100 overflow-hidden d-flex flex-column">
    ...
    <div className="flex-grow-1 overflow-auto">
      ...
      {activeTab === 'collection' && draft.type === 'SlideMenuComponent' && <div className="p-4"><h5>Slide Menu Data Source</h5><p className="text-muted small">เลือกแหล่งข้อมูลของเมนู โดยค่าทั้งหมดเป็น property ของ Component instance นี้</p>
        <div className="btn-group mb-3"><button className={`btn btn-sm ${draft.props?.dataSourceMode !== 'collection' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setDraft({ ...draft, props: { ...draft.props, dataSourceMode: 'fixed-json' } })}>1. Fixed JSON</button><button className={`btn btn-sm ${draft.props?.dataSourceMode === 'collection' ? 'btn-primary' : 'btn-outline-primary'}`} onClick={() => setDraft({ ...draft, props: { ...draft.props, dataSourceMode: 'collection' } })}>2. Collections</button></div>
      ...
      {activeTab === 'collection' && draft.type !== 'SlideMenuComponent' && <div className="p-4"><h5>Collection Source</h5><p className="text-muted small">เลือก collection ที่ component นี้จะอ่านข้อมูล</p><select className="form-select" value={String(draft.props?.collectionId || '')} onChange={(event) => setDraft({ ...draft, props: { ...draft.props, collectionId: event.target.value || undefined, dataSource: event.target.value ? { ...(draft.props?.dataSource || {}), collectionId: event.target.value } : draft.props?.dataSource } })}><option value="">ไม่ผูก Collection</option>{collections.map((collection) => <option key={collection.id} value={collection.id}>{collection.name} ({collection.table})</option>)}</select></div>}
