...
import { ArrowLeft, Database, Image as ImageIcon, LayoutTemplate, Plus, Save, Workflow } from 'lucide-react';
import { GenPageFromImageWizard, type GenPageFromImageRequest } from './GenPageFromImageWizard';
...
interface PageSettingsWorkspaceProps {
  ...
  onGenerateFromImage: (request: GenPageFromImageRequest) => Promise<void> | void;
}

export const PageSettingsWorkspace: React.FC<PageSettingsWorkspaceProps> = ({ page, collections, onBack, onAddSection, onSave, onGenerateFromImage }) => {
  ...
  return <div className="card border-0 shadow-sm h-100 overflow-auto">
    <div className="card-header bg-dark text-white d-flex align-items-center justify-content-between py-2"><div className="d-flex align-items-center gap-2"><button className="btn btn-sm btn-outline-light" onClick={onBack}><ArrowLeft size={14}/></button><div><strong>Page Settings</strong><div className="text-white-50 small">{page.name}</div></div></div><div className="d-flex gap-2"><button className="btn btn-warning btn-sm d-flex align-items-center gap-1" onClick={() => setShowImageWizard(true)}><ImageIcon size={13}/> สร้างหน้าจากรูปภาพ</button><button className="btn btn-success btn-sm d-flex align-items-center gap-1" onClick={commit}><Save size={13}/> Save Settings</button></div></div>
    ...
    {showImageWizard && <GenPageFromImageWizard pageTitle={page.title} onClose={() => setShowImageWizard(false)} onGenerate={onGenerateFromImage}/>}  </div>;
};
