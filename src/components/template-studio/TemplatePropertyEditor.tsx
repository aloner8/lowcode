...
function RecordEditor({
  ...
  return <div className="col-12">
    ...
    {entries.map(([key, item], index) => <div className="row g-2 mb-2" key={`property-row-${index}`}>
      <div className="col-md-5"><input aria-label={`${label} key`} className="form-control form-control-sm font-monospace" defaultValue={key} onBlur={(event) => onChange(renameRecordKey(value, key, event.target.value))} /></div>
...
function BindingEditor({
  ...
  return <div className="col-12">
    ...
    {entries.map(([key, item], index) => <div className="row g-2 mb-2" key={`binding-row-${index}`}>
      <div className="col-md-5"><input aria-label="Bindings key" className="form-control form-control-sm font-monospace" defaultValue={key} onBlur={(event) => onChange(renameRecordKey(value, key, event.target.value))} placeholder="Component prop" /></div>
      ...
      <div className="card-body">
        <div className="row g-3">
          ...
          {object.objectType === "MODULE" && <>
            ...
            {moduleKey === "auth" && <><div className="col-md-7"><label className="form-label small d-block">Providers</label>{["local", "google", "line", "facebook", "ldap", "ad-ds", "entra"].map((provider) => { const selected = Array.isArray(moduleConfig.providers) && moduleConfig.providers.includes(provider); return <label className="form-check form-check-inline" key={provider}><input className="form-check-input" type="checkbox" checked={selected} onChange={(event) => { const providers = Array.isArray(moduleConfig.providers) ? moduleConfig.providers.map(String) : []; set("config", { ...moduleConfig, providers: event.target.checked ? [...providers, provider] : providers.filter((item) => item !== provider) }); }} /><span className="form-check-label text-capitalize">{provider}</span></label>; })}</div><div className="col-md-5"><label className="form-label small">After login</label><input aria-label="After login" className="form-control form-control-sm" value={String(moduleConfig.afterLogin ?? "/")} onChange={(event) => set("config", { ...moduleConfig, afterLogin: event.target.value })} /></div><div className="col-12"><label className="form-check form-switch"><input className="form-check-input" type="checkbox" checked={moduleConfig.allowRegister === true} onChange={(event) => set("config", { ...moduleConfig, allowRegister: event.target.checked })}/><span className="form-check-label">Create an App member after verified provider login</span></label><div className="small text-muted">Provider credentials stay in protected Connection SecretRefs; Studio stores provider selection only.</div></div></>}
            ...
            {moduleKey === "google" && <div className="col-12"><label className="form-label small d-block">Google features</label>{["maps", "calendar", "drive", "forms", "vision", "ai"].map((feature) => { const selected = Array.isArray(moduleConfig.features) && moduleConfig.features.includes(feature); return <label className="form-check form-check-inline" key={feature}><input className="form-check-input" type="checkbox" checked={selected} onChange={(event) => { const features = Array.isArray(moduleConfig.features) ? moduleConfig.features.map(String) : []; set("config", { features: event.target.checked ? [...features, feature] : features.filter((item) => item !== feature) }); }}/><span className="form-check-label text-capitalize">{feature}</span></label>; })}<div className="form-text">Consent, scopes and quotas remain controlled by the selected Platform binding.</div></div>}
            {moduleKey === "media" && <div className="col-12"><label className="form-label small d-block">Local media features</label>{["qr", "image"].map((feature) => { const selected = Array.isArray(moduleConfig.features) && moduleConfig.features.includes(feature); return <label className="form-check form-check-inline" key={feature}><input className="form-check-input" type="checkbox" checked={selected} onChange={(event) => { const features = Array.isArray(moduleConfig.features) ? moduleConfig.features.map(String) : []; set("config", { features: event.target.checked ? [...features, feature] : features.filter((item) => item !== feature) }); }}/><span className="form-check-label text-capitalize">{feature}</span></label>; })}</div>}
