...
Output records migration/image fingerprints and measured results without row contents. Exit 0 means this synthetic rehearsal passed, not that an App is safe to migrate. It does not verify roles, external files, actual App snapshots, renderer behavior or production recovery time.
...
Sequence follow-up verified 2026-09-13: 42 tables / 51 rows and 3 sequences matched after restore; P1/P3 acceptance and FK enforcement still passed. Sequence comparison includes `last_value`, `is_called`, start/increment/min/max/cache/cycle settings. The fixture consumes then deletes an identity value so rebuilding from MAX(id) would be wrong: the restored default insert must generate 114, not reuse 107. A separate never-used sequence must first produce 500, verifying that `is_called=false` survives. These probes run after fingerprints are compared and only modify the disposable restored database. Measured run: 22.316 seconds before cleanup, same migration digest as above. Sequence ownership/role permissions and real App recovery remain outside this evidence.
